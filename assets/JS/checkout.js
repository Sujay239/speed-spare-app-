...
let demoAddresses = [
    { id: 1, name: 'Sujay', type: 'Home', line1: 'Flat 402, Sai Heights', line2: 'Madhapur, Hyderabad - 500081', phone: '+91 98765 43210', email: 'sujay@example.com', active: true },
    { id: 2, name: 'Sujay', type: 'Work', line1: 'DLF Cyber City, Phase 3', line2: 'Gachibowli, Hyderabad - 500032', phone: '+91 98765 43210', email: 'sujay@example.com', active: false }
];
...
function renderAddresses() {
    ...
    if (modalList) {
        modalList.innerHTML = demoAddresses.map(addr => `
...
window.saveNewAddress = (e) => {
    ...
    const newAddr = {
        ...
        line2: document.getElementById('new-line2').value + ', ' + document.getElementById('new-city').value + ' - ' + document.getElementById('new-pincode').value,
...
function renderOrderItems() {
    ...
    if (!container) return;
    if (cart.length === 0) {
        container.innerHTML = '<p style="text-align:center; padding:20px; color:#999;">Cart is empty</p>';
        return;
    }
...
window.togglePayment = (type) => {
    ...
    const activeCard = document.querySelector(`.payment-card input[value="${type}"]`).closest('.payment-card');
...
function updateSummary() {
    const cart = JSON.parse(localStorage.getItem('speed_spare_cart')) || [];
    let subtotal = 0;
    cart.forEach(item => subtotal += (item.newPrice || item.price) * item.quantity);
...
function showSuccessOverlay(cart) {
    ...
    finalItemsList.innerHTML = cart.map(item => `
